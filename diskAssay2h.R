...
oneParamPlot <- function(projectName, parameter, yLabel, width = 8, height = 4, ymax = 24, ymin=0, pch=19, colours =c("black", "black"), mean = TRUE, labels="default", showLines = FALSE, type=TRUE, nameOrientation="horizontal", legend=FALSE, legendText = "default", legendLoc = "bottomleft", nameVector="default", reverseY = TRUE, overwrite = TRUE){	
	...
	if (!file.exists(newdir2)){		
		dir.create(newdir2, showWarnings = TRUE)
		print(paste("Creating new directory: ", newdir2), sep="")
		}
	...
	if (!mean){
		...
		plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, ".pdf", sep="")	
		if (!overwrite){
			if (file.exists(plot1)){
						plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, "_2.pdf", sep="")	
				if (file.exists(plot1)){
					k <- 2
					while(file.exists(plot1)){
						k <- k+1
						plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, "_",k, ".pdf", sep="")	
	...
	if (mean==TRUE){
		...
		if(!is.logical(type)){
			df["type"] <- df[type]}	
		paramse <- data[,paste("se.", parameter,  sep="")]
		plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, "_ave.pdf", sep="")	
		if (!overwrite){
			if (file.exists(plot1)){
						plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, "_ave_2.pdf", sep="")	
				if (file.exists(plot1)){
					k <- 2
					while(file.exists(plot1)){
						k <- k+1
						plot1 <- paste("figures/", Sys.Date(), "_", projectName , "/", Sys.Date(), "_", projectName, "_", parameter, "_ave_",k, ".pdf", sep="")	
	...
	pdf(plot1, width=width, height=height,  pointsize=12, family="Helvetica")
	if(mean){
		plot(jitter(1: length(unique(data$lines))), parameter, ylim=c(ymin, ymax), yaxt="n", xaxt="n", ylab=yLabel, xlab="", cex=1.1, col=colours, pch=pch)
	}
	if(!mean){
		plot(jitter(as.numeric(as.factor(data$lines))), parameter, ylim=c(ymin, ymax), yaxt="n", xaxt="n", ylab=yLabel, xlab="", cex=1.1, col=colours, pch=pch)
	}
	
	axis(2, las=2)
	if(length(labels) == length(parameter)){
		axis(1, at=1:length(parameter), labels=labels, cex.axis=1)
		}
	if(showLines){
		abline(h=19, lty=2)
		abline(h=14, lty=2)
	...
	if(mean){
		arrows(1:16, parameter-paramse, 1:16, parameter+paramse, length=0)
	}
	if (nameOrientation == "horizontal"){
		...
		if (nameVector !="default"){
			axis(1, at=c(1:length(unique(data[,"lines"]))), labels= nameVector, cex.axis=0.9)}
	}
		
	if (nameOrientation == "angle"){	
		if (nameVector =="default"){
			axis(1, at=1:length(unique(data[,"lines"])), labels=FALSE)
			text(1:length(unique(data[,"lines"])), -200, srt = -45, adj = 0, labels = unique(df$lines), xpd = NA, 1.1)
		}
		if (nameVector !="default"){
			axis(1, at=1:length(unique(data[,"lines"])), labels=FALSE)
			text(1:length(unique(data[,"lines"])), -200, srt = -45, adj = 0, labels = nameVector, xpd = NA, 1.1)
		}
	}
	
	if(legend){
		if (legendText[1] != "default"){
			legend(legendLoc[1], col = unique(colours), pch=19, legend= legendText, cex= 1.1)
		}
		else{
			legend(legendLoc[1], col = unique(as.numeric(as.factor(df[,"type"]))), pch=21, legend= unique(data[,"type"]), cex=1.1)
		}
		}
